...
# Need to redefine these so they have facing direction and height parameters
...
# Change to gamepad and mouse interface so it is more like FFT.
# Possibly move to separate file for in game option changes.
...
# Need more of a team=>characters setup. one team per player, consistiong of 5-6 characters
...
# Teams will consist of 5-6 characters (adventurers), not counting henchmen/animal companions/familiers/etc. Will determine if color of team is player set or a default color scheme.
...
#will need to expand character def to include all parameters i.e.: attributes, skills, and other identifiers.
...
# Will have to change instant removal of a character for a status/state variable (i.e.: dying 0 to -9 hp, dead -10 hp, possibly a disintegrated state as well).
...
# Need to have more references to Team=>Character then individual characters. Change CT to Initiative Order. 
...
# Expand Model loader for various effects and filters.
...
# Improve Movement to include movement speeds and other movement types.
...
# CT will be determined by Initiative Check and state modifiers.
    def charge(self, time_delta):
        self.CT += time_delta * self.efficiency
        if self.CT > 1.0:
            self.CT = 1.0

#Redefine Actions for mor of a FFT/PFSRD setup.
...
# Move "Tile" to seperate file to expand and improve tile state and modifiers. Add ref for tile info and effects.
...
# Terrain and tiles will be in same file. to make effects and ref consistant.
...
# Change World class for Battle Area and use World class for World Map. Move to seperate file (possibly Tile/Terrain)
